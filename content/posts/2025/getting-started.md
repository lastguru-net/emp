...
- **/assets**: Stores shared files for your site. You can modify or add files here. Visitors can access these files via direct links.
  - **css**: Contains CSS files. The `site.css` file is generated by Eleventy (`content/utils/css.njk`) and then processed by Tailwind CSS to produce the final `site-build.css`, which is used to style the site.
