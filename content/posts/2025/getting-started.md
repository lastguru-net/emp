...
- **/assets**: Stores shared files for your site. You can modify or add files here. Visitors can access these files via direct links.
  - **css**: Contains CSS files. The `site.css` file is generated by Eleventy (`content/utils/css.njk`) and then processed by Tailwind CSS to produce the final `min.css`, which is used to style the site.
  - **js**: Contains JavaScript files. The `min.js` file is generated by Eleventy (`content/utils/js.njk`) by inlining feature scripts.
...
- Any font used here **must** exist in visitors' browsers. Unless browser default built-in fonts are used (usually a bad idea, as it is impossible to guarantee consistency across devices), the fonts must be loaded somehow. EMP supports loading Google Fonts through `/content/_data/siteelements.json5` configuration, and it is the recommended option in most cases. The names of the fonts specified in both places must match exactly.
...
This file is processed by Tailwind CSS to generate the final `min.css` file, which is used to style the site. You can extend or override these variables to customize the theme further.
